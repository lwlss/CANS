...
from cans2.process import read_in_json, remove_edges, calc_b, calc_N_0, least_sq, find_best_fits
...
log_N_0 = calc_N_0(**qfa_R_params)
log_b = calc_b(**qfa_R_params)
...
# plotter.plot_qfa_R_logistic_fit(log_plate, qfa_R_params, (4, 17), 3, 3)
...
model = next((m for m in models if m.name == comp_data["model"]), None)
...
comp_plate.set_rr_model(model, comp_data['comp_est'])
comp_plate.sim_params = comp_data["comp_est"]

# Plot the guess and fit on the same Figure.
plates = [comp_plate, comp_plate]
plate_names = ["", ""]
est_params = [comp_data["comp_est"], comp_data["init_guess"]]
models = [CompModelBC(), CompModelBC()]
plot_types = ["Competition", "Guessed"]
coords = (4, 17)
rows, cols = 3, 3
# rows, cols = 12, 20
# coords = (2, 2)
...
plot_title = r'Comparison of model fits to P15 (R5, C18)' # for \textit{cdc13-1} P15 at 27C (R5, C18)'
plotter = Plotter(model, lw=3.0, ms=12.0, mew=2.0, xpad=5, ypad=17,
                  font_size=30.0, title_font_size=38.0,
                  legend_font_size=16, labelsize=16,
                  fig_settings=fig_settings, legend_cols=3, bbox=(1.0, 0.0))
plotter.plot_zone_est(plates, plate_names, est_params, models, coords,
                      ...
                      log_plate=log_plate, log_params=qfa_R_params)
