...
Chemical reactions in well stirred mixtures are commonly modelled using mass action
kinetics, where the rate of a reaction is proportional to the product of the
concentrations of the reactants. This reflects the probabilistic nature of reactions,
which are dependent on collisions. If species numbers are large enough, reactions can be
...
required for a cell to divide and [~] are concentrations. The assumption of a well stirred
reaction mixture is perhaps not valid for a culture growing on agar. However, mass action
kinetics been used with some success to model spatially compartmentalised reactions inside
cells (ref) and animal populations (ref) where this assumption is also questionable.
...
Genetic screening using cultures grown on solid agar have important uses in functional
genomics.
