...
\ref{sec:genetic-interaction}. (r and Z)

An initial attempt to model competition for nutrients in terms of
parameters of the logistic growth model uses a time dependent
carrying capacity,
\begin{equation}
  \label{eq:5}
  K_{i}(t) = K_{0,i}\left(\frac{N_{0} + \Delta N_{i}(t)}{N_{0}}\right),
\end{equation}
where \(K_{0}\) is the carrying capacity under the assumption of
independence and is modified by the ratio of net nutrients available
to the culture at time \(t\) to the initial amount of nutrients
available under the assumption of independence. In this model, rate
constants \(r_{i}\) in \ref{eq:1} are unaffected. \(\Delta N_{i}(t)\),
the integral of the second term in \ref{eq:3}b, would need to be found
first, by fitting a nutrient only version of \ref{eq:3}, before being
used to fit \ref{eq:5}. It would take further work to find exact
relationships between parameters of \ref{eq:1} and \ref{eq:3}. ?Conor,
